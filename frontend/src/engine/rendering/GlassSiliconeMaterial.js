...
export class GlassSiliconeMaterial extends THREE.ShaderMaterial {
    constructor(color = 0x00f2ff) {
        const uniforms = {
            ...
            uWobbleIntensity: { value: 0.06 },  // reducido: menos distorsión orgánica
            uOpacity: { value: 0.72 }
        };
        ...
        const fragmentShader = `
            ...
            void main() {
                ...
                // Gelatinous interior color — pulso más lento y sutil
                vec3 baseColor = uColor * (0.6 + 0.4 * sin(uTime * 0.08));
                vec3 finalColor = mix(baseColor, vec3(1.0), fresnel * 0.25);
...
export function createSafeGlassMaterial(color) {
    try {
        ...
        return new THREE.MeshStandardMaterial({
            ...
            opacity: 0.75,
            emissive: color,
            emissiveIntensity: 0.06  // reducido: no brilla por sí mismo
        });
    }
}
